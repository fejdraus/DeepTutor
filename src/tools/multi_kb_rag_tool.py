...
from src.services.embedding.client import get_embedding_client, reset_embedding_client
...
def get_rerank_config() -> dict:
    """Get rerank configuration from environment variables."""
    return {
        "model": os.getenv("RERANK_MODEL", "qwen3-rerank"),
        "api_key": os.getenv("RERANK_API_KEY"),
        "base_url": os.getenv("RERANK_HOST"),
    }
...
async def _query_single_kb(
    ...
    try:
        ...
        with LightRAGLogContext(scene="multi_kb_rag"):
            ...
            # Pass top_k as a query parameter, not initialization parameter
            answer = await rag.aquery(query, mode=mode, only_need_context=True, top_k=top_k)
...
def _parse_context_to_chunks(content: str, kb_name: str) -> list[dict]:
    ...
    # Keep the entire context as one chunk per KB
    # This preserves the full context from each knowledge base
    # and avoids over-splitting LightRAG's formatted output
    return [{
...
async def multi_kb_rag_search(
    ...
    **kwargs,
    ...
    # Override with provided parameters (configs are dataclass objects, use . access)
    ...
    # Initialize embedding client (uses DeepTutor's unified embedding system)
    ...
    # Define embedding function using DeepTutor's unified embedding client
    # This ensures consistent dimensions and proper API configuration
    ...
    if return_detailed:
        ...
        if top_chunks:
            # Build prompt for LLM to answer based on context
...
Answer in the same language as the question."""
            ...
            user_prompt = f"""Context from knowledge bases:
{merged_context}
...
Please provide a comprehensive answer based on the context above."""
            ...
            try:
                answer = await llm_model_func(user_prompt, system_prompt=system_prompt)
            except Exception as e:
        ...
        return {
            "query": query,
            "answer": answer,
            "context": merged_context,  # Also include raw context for reference
            "mode": mode,
            "source_kbs": successful_kbs,
        }
...
if __name__ == "__main__":
    ...
    async def test():
        result = await multi_kb_rag_search(
            query="What do COMP courses teach?",
        ...
        # return_detailed=True 时只有 results，没有 answer
        if result.get("results"):
            print(f"Results ({len(result['results'])}):")
            for r in result["results"]:
                print(f"  [{r['source_kb']}] score={r['rerank_score']:.3f}: {r['content']}")
